...
// 从 chat.logic.js 中导入消息存储辅助函数
// Import message storage helper from chat.logic.js
import {
	appendMessage
} from './chat.logic.js';
		...
		if (rd && rd.chat) {
			...
			try {
				...
				if (rd.privateChatTargetId) {
					...
					const sent = await rd.chat.sendClientMessage(targetClientId, 'image_private', messageContent);
					if (!sent) {
				...
				if (rd.privateChatTargetId) {
					...
					const sent = await rd.chat.sendClientMessage(targetClientId, 'text_private', text);
					if (!sent) {
						addSystemMsg(`${t('system.private_message_failed', 'Cannot send private message to')} ${targetClientName}. ${t('system.user_not_connected', 'User might not be fully connected.')}`)
						return
					}
		...
		onSend: async (message, transferContext = {}) => {
			...
			if (rd && rd.chat) {
				...
				if (isPrivateTransfer) {
					...
					const sent = await rd.chat.sendClientMessage(targetClientId, msgWithUser.type + '_private', msgWithUser);
					if (!sent) {
