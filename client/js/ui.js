...
// Share-token obfuscation using base64 and character shifting.
// NOT encryption: the token is trivially reversible, effectively plaintext.
// 使用base64和字符偏移混淆分享令牌。并非加密：可被轻易还原，等同明文。
function obfuscateShareToken(text) {
	if (!text) return '';
...
function deobfuscateShareToken(encoded) {
	if (!encoded) return '';
	try {
		...
		console.warn('Failed to decode share token:', error);
		return '';
	}
}
...
function copyToClipboard(text, successMessage = t('action.copied', 'Copied to clipboard!'), errorPrefix = t('action.copy_failed', 'Copy failed, url:'), onCopied = null) {
	...
	if (navigator.clipboard && navigator.clipboard.writeText) {
		navigator.clipboard.writeText(text).then(() => {
			...
			showFallbackCopy(text, errorPrefix, onCopied);
		});
	} else {
		showFallbackCopy(text, errorPrefix, onCopied);
	}
...
// 显示降级复制方法
function showFallbackCopy(text, prefix, onCopied = null) {
...
function handleShareAction() {
	...
	// Obfuscate room name and password (reversible, effectively plaintext)
	const encodedRoom = obfuscateShareToken(roomName);
	const encodedPwd = password ? obfuscateShareToken(password) : '';
	
	// Keep share tokens in the fragment so they are not sent to the relay.
	const params = new URLSearchParams();
	params.set('r', encodedRoom);
	if (encodedPwd) {
		params.set('p', encodedPwd);
	}
...
export function createUserItem(user, isMe) {
	...
	if (!isMe) {
		...
		div.setAttribute('aria-pressed', isPrivateTarget ? 'true' : 'false');
		div.setAttribute('aria-label', `${t('ui.private_message_to', 'Private Message to')} ${rawName}`)
	}
	const avatarEl = div.querySelector('.avatar');
	if (avatarEl) {
		avatarEl.innerHTML = createAvatarSVG(rawName)
	}
	if (!isMe) {
		const activatePrivateChat = () => togglePrivateChat(user.clientId, rawName);
		div.onclick = activatePrivateChat;
		div.onkeydown = (event) => {
...
export function generateLoginForm(isModal = false) {
	...
	return `		<div class="input-group">
		...
		<div class="login-pwd-hint">${t('ui.password_hint', 'Nodes without a password cannot resist server-side eavesdropping. Set a strong password.')}</div>
...
export function autofillRoomPwd(formPrefix = '') {
	...
	if (encryptedRoom) {
		...
		roomValue = deobfuscateShareToken(encryptedRoom);
		if (encryptedPwd) {
			pwdValue = deobfuscateShareToken(encryptedPwd);
		}
