...
const LANGUAGES = {
	en: {
		...
		translations: {
			...
			'ui.private_chat_close': 'Exit private chat',
			'ui.connection_connecting': 'Connecting',
			'ui.connection_connected': 'Connected',
			'ui.connection_reconnecting': 'Reconnecting',
			'ui.connection_offline': 'Offline',
			'ui.me': ' (me)',
			'ui.anonymous': 'Anonymous',
			'ui.start_private_chat': 'Select for private chat',
			'ui.password_hint': 'Nodes without a password cannot resist server-side eavesdropping. Set a strong password.',
			'ui.share_link_warning': 'The share link contains the node name and password (obfuscated only, effectively plaintext). Share it only through trusted channels.',
			...
			'system.file_prepare': 'Preparing file transfer...',
			'system.message_send_failed': 'Message send failed. Please try again.',
			'system.node_not_ready': 'Node connection is not ready.',
			'system.file_unavailable': 'File data is not available. Ask the sender to resend it.',
			'system.file_not_ready': 'File is still being received. Please try again later.',
			'system.joined': 'joined the conversation',
			'system.left': 'left the conversation',
			'system.secured': 'connection secured',
			'system.connection_closed': 'Node connection closed',
			'system.connection_reconnecting': 'Connection lost. Reconnecting...',
			'system.connection_restored': 'Connection restored',
			'system.server_key_changed_confirm': 'The server identity key changed. This can happen after a server redeploy, but it can also indicate interception. Trust the new key?',
			'system.server_key_accepted': 'New server identity key trusted',
			'system.server_key_rejected': 'Server identity key rejected',
			'system.server_trust_error': 'Server identity verification failed',
			'system.history_loaded_count': '{count} historical messages loaded',
			...
			'help.what_is_nodecrypt': '🔐 What is NodeCrypt?',			'help.what_is_nodecrypt_desc': 'NodeCrypt is a true zero-knowledge end-to-end encrypted chat system. With a plaintext-message-database-free architecture, all messages are encrypted locally on your device, and the server serves only as an encrypted data relay station, unable to access any of your plaintext content.',
			...
			'help.no_history': '🕒 Temporary Room History',
			'help.no_history_desc': 'Text history is encrypted locally and cached only while the room has active members; it is cleared when everyone leaves',
	...
	zh: {
		...
		translations: {
			...
			'ui.private_chat_close': '退出私聊',
			'ui.connection_connecting': '连接中',
			'ui.connection_connected': '已连接',
			'ui.connection_reconnecting': '重连中',
			'ui.connection_offline': '离线',
			'ui.me': '（我）',
			'ui.anonymous': '匿名用户',
			'ui.start_private_chat': '选择用户开始私信',
			'ui.password_hint': '无密码的节点无法防御服务端窃听，建议设置高强度密码',
			'ui.share_link_warning': '分享链接包含节点名与密码（仅混淆、等同明文），请只通过可信渠道发送',
			...
			'system.file_prepare': '正在准备文件传输...',
			'system.message_send_failed': '消息发送失败，请重试。',
			'system.node_not_ready': '节点连接尚未就绪。',
			'system.file_unavailable': '文件数据不可用，请让发送方重新发送。',
			'system.file_not_ready': '文件仍在接收中，请稍后再试。',
			'system.joined': '加入了对话',
			'system.left': '离开了对话',
			'system.secured': '已建立端到端安全连接',
			'system.connection_closed': '节点连接已关闭',
			'system.connection_reconnecting': '连接已断开，正在重连...',
			'system.connection_restored': '连接已恢复',
			'system.server_key_changed_confirm': '服务端身份密钥发生变化。这可能来自服务端重新部署，也可能表示连接被拦截。是否信任新密钥？',
			'system.server_key_accepted': '已信任新的服务端身份密钥',
			'system.server_key_rejected': '已拒绝服务端身份密钥',
			'system.server_trust_error': '服务端身份验证失败',
			'system.history_loaded_count': '已加载 {count} 条历史消息',
			...
			'help.what_is_nodecrypt': '🔐 什么是 NodeCrypt？',			'help.what_is_nodecrypt_desc': 'NodeCrypt 是一个真正的端到端加密聊天系统。采用无明文消息数据库架构，所有消息在您的设备上本地加密，服务器仅作为加密数据的中转站，无法获取您的任何明文内容。',
			...
			'help.no_history': '🕒 临时房间历史',
			'help.no_history_desc': '文本历史在本地加密后仅于房间存在活跃成员期间缓存，所有成员离开后即清空',
